import {
  getInstanceAuthenticatedUser,
  getInstanceRuntime,
  getInstanceSystemStatus,
  getRuntimeContext,
} from "./runtime-context.js";
import {
  ANTHROPIC_CHANNEL_TYPE,
  buildSequentialChannelNames,
  CHANNEL_KINDS,
  CLAUDE_MODELS,
  GROK_MODELS,
  OPENAI_CHANNEL_TYPE,
  OPENAI_MODELS,
  redactSensitiveText,
  validateChannelDefaults,
  validateImportInput,
  ValidationError,
  XAI_CHANNEL_TYPE,
} from "./validation.js";

const DEFAULT_NEW_API_QUOTA_PER_UNIT = 500_000;
const CHANNEL_SYNCHRONIZATION_CONCURRENCY = 5;
const CLAUDE_CHANNEL_GROUP = "anthropic";
const OPENAI_CHANNEL_GROUP = "openai";
const GROK_CHANNEL_GROUP = "xai";
const CHANNEL_DEFINITIONS = Object.freeze({
  [CHANNEL_KINDS.CLAUDE]: Object.freeze({
    type: ANTHROPIC_CHANNEL_TYPE,
    models: CLAUDE_MODELS,
    group: CLAUDE_CHANNEL_GROUP,
    platformChannelType: "anthropic_claude",
    modelSeries: "anthropic.claude",
  }),
  [CHANNEL_KINDS.OPENAI]: Object.freeze({
    type: OPENAI_CHANNEL_TYPE,
    models: OPENAI_MODELS,
    group: OPENAI_CHANNEL_GROUP,
    platformChannelType: "openai",
    modelSeries: "openai.gpt",
  }),
  [CHANNEL_KINDS.GROK]: Object.freeze({
    type: XAI_CHANNEL_TYPE,
    models: GROK_MODELS,
    group: GROK_CHANNEL_GROUP,
  }),
});

function getQuotaPerUnit(systemStatus) {
  const configuredQuotaPerUnit = Number(systemStatus?.quota_per_unit);
  return Number.isFinite(configuredQuotaPerUnit) && configuredQuotaPerUnit > 0
    ? configuredQuotaPerUnit
    : DEFAULT_NEW_API_QUOTA_PER_UNIT;
}

function getValidatedInstanceDefaults(instance) {
  return validateChannelDefaults({
    group: instance.group,
    namePrefix: instance.namePrefix,
    startNumber: instance.startNumber,
    continueFromExisting: instance.continueFromExisting,
    priority: instance.priority,
    weight: instance.weight,
    dateMode: instance.dateMode,
  });
}

export function prepareChannelImport(
  instanceId,
  rawKeys,
  requestId,
  rawChannelKind = CHANNEL_KINDS.CLAUDE,
  rawGroup = null,
) {
  const context = getRuntimeContext();
  const instanceRuntime = getInstanceRuntime(instanceId);
  if (!instanceRuntime) {
    throw new ValidationError("New API 实例不存在");
  }
  if (instanceRuntime.importInProgress) {
    throw new ValidationError("该 New API 实例正在执行另一批导入，请稍后再试");
  }

  const channelDefaults = getValidatedInstanceDefaults(instanceRuntime.connection);
  const normalizedChannelKind = String(rawChannelKind).trim().toLowerCase();
  if (
    normalizedChannelKind === CHANNEL_KINDS.GROK
    && instanceRuntime.connection.connectionProtocol !== "new-api"
  ) {
    throw new ValidationError("Grok 渠道仅支持标准 New API 实例");
  }
  const requestedChannelDefinition = CHANNEL_DEFINITIONS[normalizedChannelKind];
  const normalizedRequestedGroup = String(rawGroup ?? "").trim();
  const shouldValidateRequestedGroup = (
    instanceRuntime.connection.connectionProtocol === "new-api"
    && normalizedRequestedGroup.length > 0
  );
  const importInput = validateImportInput({
    keys: rawKeys,
    channelKind: normalizedChannelKind,
    group: shouldValidateRequestedGroup
      ? normalizedRequestedGroup
      : requestedChannelDefinition?.group ?? channelDefaults.group,
    namePrefix: channelDefaults.namePrefix,
    dateSegment: channelDefaults.dateSegment,
    startNumber: channelDefaults.startNumber,
    continueFromExisting: channelDefaults.continueFromExisting,
    priority: channelDefaults.priority,
    weight: channelDefaults.weight,
  });
  const channelDefinition = CHANNEL_DEFINITIONS[importInput.channelKind];
  instanceRuntime.importInProgress = true;

  return {
    importInput,
    async *events() {
      const startedAt = Date.now();
      const sensitiveValues = [
        instanceRuntime.connection.password,
        ...importInput.keys,
      ];
      let completedCount = 0;
      let successCount = 0;
      let failureCount = 0;
      context.logger.info("channel_import_started", {
        requestId,
        instanceId: Number(instanceId),
        channelKind: importInput.channelKind,
        total: importInput.keys.length,
      });

      try {
        const systemStatus = await getInstanceSystemStatus(instanceRuntime);
        const loggedInUser = await getInstanceAuthenticatedUser(instanceRuntime);
        if (shouldValidateRequestedGroup) {
          const availableGroups = await instanceRuntime.client.listAvailableGroups();
          if (!availableGroups.includes(importInput.group)) {
            throw new ValidationError(
              `所选分组 ${importInput.group} 已不存在，请刷新分组后重新选择`,
            );
          }
        }
        const channelNamePrefix = importInput.dateSegment
          ? `${importInput.namePrefix}-${importInput.dateSegment}-`
          : `${importInput.namePrefix}-`;
        const existingChannelNames = await instanceRuntime.client.listChannelNamesByPrefix(
          channelNamePrefix,
        );
        const channelNames = buildSequentialChannelNames({
          existingNames: existingChannelNames,
          keyCount: importInput.keys.length,
          namePrefix: importInput.namePrefix,
          dateSegment: importInput.dateSegment,
          startNumber: importInput.startNumber,
          continueFromExisting: importInput.continueFromExisting,
        });

        yield {
          type: "ready",
          systemName: systemStatus?.system_name || "New API",
          version: systemStatus?.version || "未知版本",
          username: loggedInUser.username,
          channelKind: importInput.channelKind,
          total: importInput.keys.length,
        };

        for (let keyIndex = 0; keyIndex < importInput.keys.length; keyIndex += 1) {
          const channelName = channelNames[keyIndex];
          let channelCreated = false;
          yield { type: "item-start", index: keyIndex, name: channelName };

          try {
            if (context.store.hasImportedKey(instanceId, importInput.keys[keyIndex])) {
              throw new ValidationError("该 Key 已通过本工具导入，请勿重复提交");
            }
            await instanceRuntime.client.createChannel({
              key: importInput.keys[keyIndex],
              name: channelName,
              group: importInput.group,
              priority: importInput.priority,
              weight: importInput.weight,
              channelType: channelDefinition.type,
              models: channelDefinition.models,
              platformChannelType: channelDefinition.platformChannelType,
              modelSeries: channelDefinition.modelSeries,
              testModel: channelDefinition.models[0],
            });
            channelCreated = true;
            const createdChannel = await instanceRuntime.client.findChannelByName(
              channelName,
              channelDefinition.type,
            );
            context.store.recordImportedChannel({
              instanceId,
              baseUrl: instanceRuntime.connection.baseUrl,
              key: importInput.keys[keyIndex],
              channel: createdChannel,
              quotaPerUnit: getQuotaPerUnit(systemStatus),
            });
            successCount += 1;
            yield {
              type: "item-result",
              index: keyIndex,
              name: channelName,
              success: true,
              message: "渠道创建成功",
            };
          } catch (error) {
            const redactedMessage = redactSensitiveText(error?.message, sensitiveValues);
            if (channelCreated) {
              successCount += 1;
              yield {
                type: "item-result",
                index: keyIndex,
                name: channelName,
                success: true,
                warning: true,
                message: `渠道已创建，但历史记录保存失败：${redactedMessage}`,
              };
            } else {
              failureCount += 1;
              context.logger.error("channel_creation_failed", {
                instanceId: Number(instanceId),
                channelKind: importInput.channelKind,
                channelName,
                error: new Error(redactedMessage),
                requestId,
              });
              yield {
                type: "item-result",
                index: keyIndex,
                name: channelName,
                success: false,
                message: redactedMessage,
              };
            }
          }
          completedCount += 1;
        }

        yield {
          type: "complete",
          total: importInput.keys.length,
          completed: completedCount,
          success: successCount,
          failure: failureCount,
        };
      } catch (error) {
        const redactedMessage = redactSensitiveText(error?.message, sensitiveValues);
        context.logger.error("channel_import_failed", {
          requestId,
          instanceId: Number(instanceId),
          channelKind: importInput.channelKind,
          error: new Error(redactedMessage),
        });
        yield {
          type: "fatal",
          message: redactedMessage,
          completed: completedCount,
          success: successCount,
          failure: failureCount,
        };
      } finally {
        instanceRuntime.importInProgress = false;
        context.logger.info("channel_import_completed", {
          instanceId: Number(instanceId),
          channelKind: importInput.channelKind,
          total: importInput.keys.length,
          success: successCount,
          failure: failureCount,
          durationMilliseconds: Date.now() - startedAt,
          requestId,
        });
      }
    },
  };
}

async function performInstanceRecordSynchronization({
  context,
  instanceId,
  instanceRuntime,
  requestId,
}) {
  const startedAt = Date.now();
  const importedRecords = context.store.listRecords(instanceId);
  if (importedRecords.length === 0) {
    const synchronization = context.store.synchronizeChannels({
      instanceId,
      channels: [],
      quotaPerUnit: 0,
    });
    context.logger.info("channel_usage_sync_completed", {
      instanceId: Number(instanceId),
      synchronizedCount: synchronization.synchronizedCount,
      missingCount: synchronization.missingCount,
      durationMilliseconds: Date.now() - startedAt,
      trackedCount: 0,
      requestId,
    });
    return synchronization;
  }

  const systemStatus = await getInstanceSystemStatus(instanceRuntime);
  await getInstanceAuthenticatedUser(instanceRuntime);
  const channels = [];
  for (
    let batchStartIndex = 0;
    batchStartIndex < importedRecords.length;
    batchStartIndex += CHANNEL_SYNCHRONIZATION_CONCURRENCY
  ) {
    const recordBatch = importedRecords.slice(
      batchStartIndex,
      batchStartIndex + CHANNEL_SYNCHRONIZATION_CONCURRENCY,
    );
    const matchingChannels = await Promise.all(recordBatch.map(async (importedRecord) => {
      const matchingChannel = await instanceRuntime.client.searchChannelByName(
        importedRecord.channelName,
      );
      return matchingChannel
        && Number(matchingChannel.id) === Number(importedRecord.newApiChannelId)
        ? matchingChannel
        : null;
    }));
    channels.push(...matchingChannels.filter(Boolean));
  }
  const synchronization = context.store.synchronizeChannels({
    instanceId,
    channels,
    quotaPerUnit: getQuotaPerUnit(systemStatus),
  });
  context.logger.info("channel_usage_sync_completed", {
    instanceId: Number(instanceId),
    synchronizedCount: synchronization.synchronizedCount,
    missingCount: synchronization.missingCount,
    durationMilliseconds: Date.now() - startedAt,
    trackedCount: importedRecords.length,
    requestId,
  });
  return synchronization;
}

export async function synchronizeInstanceRecords(instanceId, requestId) {
  const context = getRuntimeContext();
  const instanceRuntime = getInstanceRuntime(instanceId);
  if (!instanceRuntime) {
    throw new ValidationError("New API 实例不存在");
  }
  if (instanceRuntime.synchronizationPromise) {
    return instanceRuntime.synchronizationPromise;
  }

  const synchronizationPromise = performInstanceRecordSynchronization({
    context,
    instanceId,
    instanceRuntime,
    requestId,
  });
  instanceRuntime.synchronizationPromise = synchronizationPromise;
  try {
    return await synchronizationPromise;
  } finally {
    if (instanceRuntime.synchronizationPromise === synchronizationPromise) {
      instanceRuntime.synchronizationPromise = null;
    }
  }
}
