...
export function validateChannelDefaults(rawInput = {}) {
  ...
  if (dateMode !== "auto" && !/^\d{4}$/.test(dateMode)) {
    throw new ValidationError("CHANNEL_DATE_MODE 必须是 auto 或 4 位日期段");
  }
...
export function validateImportInput(rawInput) {
  ...
  const dateSegment = validateRequiredText(rawInput?.dateSegment, "日期段", 4);
  ...
  if (!/^\d{4}$/.test(dateSegment)) {
    throw new ValidationError("日期段必须是 4 位数字，例如 0711");
  }
  if (!Number.isSafeInteger(startNumber) || startNumber < 1 || startNumber > 999_999) {
...
export function buildSequentialChannelNames({
  ...
  const channelNameBase = `${namePrefix}-${dateSegment}`;
