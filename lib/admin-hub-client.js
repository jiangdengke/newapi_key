...
function getMetricValue(batchResult, fieldNames) {
  if (typeof batchResult === "number") {
    return Number.isFinite(batchResult) ? batchResult : undefined;
  }
  for (const fieldName of fieldNames) {
    if (batchResult?.[fieldName] !== undefined) {
      const metricValue = Number(batchResult[fieldName]);
      return Number.isFinite(metricValue) ? metricValue : undefined;
    }
  }
  return undefined;
}

function getUsedQuotaValue(batchResult) {
  const quotaFieldNames = ["used_quota", "usedQuota", "total_used_quota"];
  const directUsedQuota = getMetricValue(batchResult, quotaFieldNames);
  const siteMetrics = Array.isArray(batchResult?.sites) ? batchResult.sites : [];
  const siteUsedQuotaValues = siteMetrics
    .map((siteMetric) => getMetricValue(siteMetric, quotaFieldNames))
    .filter((siteUsedQuota) => siteUsedQuota !== undefined);
  const siteUsedQuota = siteUsedQuotaValues.length > 0
    ? siteUsedQuotaValues.reduce(
      (totalUsedQuota, currentUsedQuota) => totalUsedQuota + currentUsedQuota,
      0,
    )
    : undefined;

  if ((directUsedQuota === undefined || directUsedQuota <= 0) && siteUsedQuota > 0) {
    return siteUsedQuota;
  }
  return directUsedQuota ?? siteUsedQuota;
}
...
export class AdminHubClient extends NewApiClient {
  ...
  async getChannelMetrics(channelId) {
    ...
    return {
      ...
      usedQuota: getUsedQuotaValue(usedQuotaResult),
    };
  }
  ...
  async searchChannelByName(channelName, channelType = null) {
    ...
    const matchingTemplate = getChannelItems(successfulPayload).find((channelTemplate) => {
      ...
      return channel.name === channelName
        && (channelType === null || channel.type === Number(channelType));
    });
    if (!matchingTemplate) {
      return null;
    }
  ...
  async findChannelByName(channelName, channelType) {
    const matchingChannel = await this.searchChannelByName(channelName, channelType);
    if (!matchingChannel) {
      throw new NewApiRequestError(
  ...
  async searchAnthropicChannelByName(channelName) {
    return this.searchChannelByName(channelName, ANTHROPIC_CHANNEL_TYPE);
  }

  async findAnthropicChannelByName(channelName) {
    return this.findChannelByName(channelName, ANTHROPIC_CHANNEL_TYPE);
  }
  ...
  async createChannel({
    ...
    const targetSiteId = this.requireTargetSiteId();
    const channel = {
      platform_channel_type: platformChannelType,
      type: channelType,
      key,
      name,
      models: models.join(","),
